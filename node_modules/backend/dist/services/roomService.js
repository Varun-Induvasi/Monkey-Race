...
export function createRoom(hostSocketId, userId, username, avatar) {
...
export function joinRoom(code, socketId, userId, username, avatar, isSpectator = false) {
    ...
    const existingPlayerSocketId = Object.keys(room.players).find(sid => room.players[sid].userId === userId);
...
export function updateRoomSettings(code, hostSocketId, settings) {
    const room = rooms.get(code.toUpperCase());
    if (!room)
        throw new Error('Room not found');
    ...
    if (room.status !== 'waiting')
        throw new Error('Cannot change settings while race is in progress');
...
export function togglePlayerReady(code, socketId) {
    const room = rooms.get(code.toUpperCase());
    if (!room)
        throw new Error('Room not found');
    ...
    if (player.isSpectator)
        return room; // spectators are always ready
    player.ready = !player.ready;
    return room;
}
export function updatePlayerProgress(code, socketId, progress, wpm, rawWpm, accuracy, errorCount, correctChars, incorrectChars, finished, timeTakenMs, timelineEntry) {
    const room = rooms.get(code.toUpperCase());
    if (!room)
        throw new Error('Room not found');
    const player = room.players[socketId];
    if (!player)
        throw new Error('Player not in room');
    if (player.isSpectator)
        throw new Error('Spectators cannot update progress');
