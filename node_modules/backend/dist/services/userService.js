...
export async function createUser(username, email, password) {
    ...
    await db.run('INSERT INTO users (id, username, email, password_hash, avatar) VALUES (?, ?, ?, ?, ?)', [userId, username, email, passwordHash, avatar]);
    return userId;
}
...
export async function saveMatchAndResults(roomCode, textContent, results) {
    const db = await getDb();
    ...
    for (const res of results) {
        ...
        await db.run(`INSERT INTO player_results (
        ...
        if (res.userId !== 'guest') {
            await db.run('UPDATE users SET xp = xp + ?, coins = coins + ? WHERE id = ?', [res.xpGained, Math.floor(res.xpGained / 2), res.userId]);
...
export async function checkAndUnlockAchievements(userId) {
  ...
  `, [userId]);
    ...
    const unlock = async (key) => {
        try {
            await db.run('INSERT INTO achievements (id, user_id, achievement_key) VALUES (?, ?, ?)', [uuidv4(), userId, key]);
    ...
    if (totalWins >= 1)
        await unlock('first_victory');
    if (totalWins >= 10)
        await unlock('wins_10');
    if (totalRaces >= 10)
        await unlock('races_10');
    if (totalRaces >= 100)
        await unlock('races_100');
    if (maxWpm >= 100)
        await unlock('wpm_100');
    if (maxWpm >= 120)
        await unlock('wpm_120');
    if (maxWpm >= 150)
        await unlock('wpm_150');
    if (maxAccuracy >= 99)
        await unlock('accuracy_master');
    return unlockedNow;
}
// Get standard ranking badges based on XP
export function getRankBadge(xp) {
    if (xp >= 15000)
        return { tier: 'Grand Master', color: '#ff4500' };
    if (xp >= 8000)
        return { tier: 'Master', color: '#da70d6' };
    if (xp >= 4000)
        return { tier: 'Diamond', color: '#00ffff' };
    if (xp >= 2000)
        return { tier: 'Platinum', color: '#afeeee' };
    if (xp >= 1000)
        return { tier: 'Gold', color: '#ffd700' };
    if (xp >= 400)
        return { tier: 'Silver', color: '#c0c0c0' };
