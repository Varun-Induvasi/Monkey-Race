import { createRoom, joinRoom, leaveRoom, getRoom, updateRoomSettings, togglePlayerReady, updatePlayerProgress } from './services/roomService.js';
...
export function setupSockets(io) {
    io.on('connection', (socket) => {
        ...
        socket.on('update-progress', ({ code, progress, wpm, rawWpm, accuracy, errorCount, correctChars, incorrectChars, finished, timeTakenMs, timelineEntry }) => {
            try {
                const room = getRoom(code);
                if (!room || room.status !== 'racing')
                    return;
                const updatedRoom = updatePlayerProgress(code, socket.id, progress, wpm, rawWpm, accuracy, errorCount, correctChars, incorrectChars, finished, timeTakenMs, timelineEntry);
...
async function endRace(io, room) {
    ...
    const typingPlayers = Object.values(room.players).filter((p) => !p.isSpectator);
    // Sort players by progress (descending) and then timeTakenMs (ascending) to decide rankings
    const rankedPlayers = [...typingPlayers].sort((a, b) => {
    ...
    const results = rankedPlayers.map((player, idx) => {
        const rank = idx + 1;
